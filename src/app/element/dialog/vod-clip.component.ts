...
export class VodClipDialogComponent {
  ...
  public isPreviewCreating = false;
    ...
    this.endTimeCtrl = new FormControl('0:0', {
      validators: [
        ...
        (control) => {
          ...
          const startSec =
            parseInt(this.startTimeCtrl.value.split(':')[0]) * 60 +
            parseInt(this.startTimeCtrl.value.split(':')[1]);
          const endSec =
            parseInt(control.value.split(':')[0]) * 60 +
            parseInt(control.value.split(':')[1]);
  ...
  private convertTimeToSec(time: string) {
    const s = time.split(':');
