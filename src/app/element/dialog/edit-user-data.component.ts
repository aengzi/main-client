...
export class EditUserDataDialogComponent {
  ...
  public constructor(
    ...
    dialogRef: MatDialogRef<EditUserDataDialogComponent>
  ) {
  ...
  public ngAfterViewInit() {
    ...
    import('src/app/element/user-data/edit/' + this.label + '.module')
      .then((editModule) => {
        return this.compiler.compileModuleAndAllComponentsAsync(
          editModule[editMdlName]
        );
      })
      .then((editModule) => {
        this.editContent.clear();
        const editCmpFactory = _.find((<any>editModule).componentFactories, [
          'componentType.name',
          editCmpName,
        ]);
        const editCmp = <any>(
          this.editContent.createComponent(editCmpFactory).instance
        );
